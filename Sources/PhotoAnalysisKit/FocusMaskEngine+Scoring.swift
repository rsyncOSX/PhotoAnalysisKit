...
extension FocusMaskEngine {
    nonisolated func computeSharpnessScore(
        ...
        afPoint: CGPoint? = nil
    ) async -> (score: Float?, saliency: SaliencySummary?, breakdown: SharpnessBreakdown?) {
        await Self.runCancellableWorker { [context] in
            guard !Task.isCancelled else { return (nil, nil, nil) }
            guard let normalizedImage = Self.normalizeToSRGB(cgImage) else {
                return (nil, nil, nil)
            }
    ...
    nonisolated static func detectSaliencyAndClassify(for cgImage: CGImage, classify: Bool) -> SaliencyDetection {
        ...
        return SaliencyDetection(
            ...
            saliencyInfo: candidates.isEmpty ? nil : SaliencySummary(subjectLabel: label, subjectConfidence: maxConfidence),
        )
    }
    ...
    private nonisolated static func buildScoringLaplacian(from image: CIImage, config: SharpnessConfiguration) -> CIImage? {
        var scoringConfig = config
        scoringConfig.energyMultiplier = SharpnessConfiguration.stableScoringEnergyMultiplier
